<#
.SYNOPSIS
    Smoke tests for the TomTom MCP Server with Orbis Maps backend.

.DESCRIPTION
    Runs a comprehensive suite of smoke tests against the deployed TomTom MCP Server
    configured with the Orbis Maps backend. Tests all 16 tools including the 5 new
    Orbis-specific tools: EV routing, search along route, area search, EV charging
...
.PARAMETER TestFilter
    ...
    Example: -TestFilter "health*" or -TestFilter "*ev*"

.EXAMPLE
    .\Invoke-OrbisTests.ps1 -BaseUrl "https://ca-tomtom-mcp.azurecontainerapps.io"
    .\Invoke-OrbisTests.ps1 -BaseUrl "https://ca-tomtom-mcp.azurecontainerapps.io" -TestFilter "*ev*"
    .\Invoke-OrbisTests.ps1 -BaseUrl "https://myserver.azurecontainerapps.io" -ApiKey "MY_KEY"
#>
...
$Tests = @(
    ...
    @{
        Name = "tools-list"
        Description = "MCP tools/list returns all 16 Orbis tools"
        Run = {
            ...
            if ($result.Success) {
                ...
                $expectedTools = @(
                    ...
                    "tomtom-dynamic-map", "tomtom-ev-routing", "tomtom-search-along-route",
                    "tomtom-area-search", "tomtom-ev-search", "tomtom-data-viz",
                    "tomtom-waypoint-optimization"
                )
                $missingTools = $expectedTools | Where-Object { $_ -notin $toolNames }
                $passed = ($missingTools.Count -eq 0) -and ($tools.Count -ge 16)
                $msg = if ($passed) { "Found $($tools.Count) tools (all 16 Orbis tools present)" } else { "Found $($tools.Count) tools. Missing: $($missingTools -join ', ')" }
                Write-TestResult -TestName "Tools List (Orbis)" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "Tools List (Orbis)" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
    ...
    @{
        Name = "ev-routing"
        Description = "EV route from London to Edinburgh with battery parameters"
        Run = {
            $result = Invoke-McpRequest -Method "tools/call" -Params @{
                ...
                arguments = @{
                    ...
                    constantSpeedConsumptionInkWhPerHundredkm = "50,8.2:100,11.5:130,17.5"
                }
            } -TimeoutSec 60
            if ($result.Success) {
                $content = $result.Data.result.content[0].text
                $passed = ($null -ne $content) -and ($content.Length -gt 50)
                $msg = "Response length: $($content.Length) chars"
                # Try to parse for route details
                try {
                    $parsed = $content | ConvertFrom-Json
                    if ($parsed.routes) {
                        $route = $parsed.routes[0]
                        $distKm = [math]::Round($route.summary.lengthInMeters / 1000, 1)
                        $msg = "Distance: ${distKm}km, Charging stops included"
                    }
                }
                catch { }
                Write-TestResult -TestName "EV Routing - London to Edinburgh" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "EV Routing - London to Edinburgh" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
        Name = "search-along-route"
        Description = "Search for restaurants along a route from Cardiff to London"
        Run = {
            $result = Invoke-McpRequest -Method "tools/call" -Params @{
                ...
                arguments = @{
                    ...
                    route      = @(
                        ...
                        @{ lat = 51.5850; lon = -2.9970 },
                        @{ lat = 51.5500; lon = -2.5900 },
                        @{ lat = 51.4500; lon = -2.2000 },
                        @{ lat = 51.5074; lon = -0.1278 }
                    )
                    maxDetourTimeInSeconds = 600
                    limit = 5
                }
            } -TimeoutSec 45
            if ($result.Success) {
                $content = $result.Data.result.content[0].text
                $passed = ($null -ne $content) -and ($content.Length -gt 20)
                $msg = "Response length: $($content.Length) chars"
                try {
                    $parsed = $content | ConvertFrom-Json
                    if ($parsed.results) {
                        $msg = "Results: $($parsed.results.Count) restaurants found along route"
                    }
                    elseif ($parsed.summary) {
                        $msg = "Results: $($parsed.summary.numResults) restaurants found along route"
                    }
                }
                catch { }
                Write-TestResult -TestName "Search Along Route - Restaurants" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "Search Along Route - Restaurants" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
        Name = "area-search"
        Description = "Search within a bounding box in central London"
        Run = {
            $result = Invoke-McpRequest -Method "tools/call" -Params @{
            ...
            if ($result.Success) {
                ...
                try {
                    $parsed = $content | ConvertFrom-Json
                    if ($parsed.results) {
                        $msg = "Results: $($parsed.results.Count) coffee shops in bounding box"
                    }
                    elseif ($parsed.summary) {
                        $msg = "Results: $($parsed.summary.numResults) coffee shops in bounding box"
                    }
                }
                catch { }
                Write-TestResult -TestName "Area Search - Coffee Shops London" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "Area Search - Coffee Shops London" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
        Name = "ev-search"
        Description = "Find EV charging stations near London"
        Run = {
            $result = Invoke-McpRequest -Method "tools/call" -Params @{
            ...
            if ($result.Success) {
                ...
                try {
                    $parsed = $content | ConvertFrom-Json
                    if ($parsed.results) {
                        $stationCount = $parsed.results.Count
                        $msg = "Found $stationCount EV charging stations near London"
                    }
                    elseif ($parsed.summary) {
                        $msg = "Found $($parsed.summary.numResults) EV charging stations near London"
                    }
                }
                catch { }
                Write-TestResult -TestName "EV Search - Charging Stations London" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "EV Search - Charging Stations London" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
        Name = "data-viz"
        Description = "Data visualization with simple GeoJSON"
        Run = {
            $geojson = @{
            ...
            $result = Invoke-McpRequest -Method "tools/call" -Params @{
                ...
                arguments = @{
                    geojson = $geojson
                    title   = "Test Visualization - UK Cities"
                }
            } -TimeoutSec 45
            if ($result.Success) {
                $content = $result.Data.result.content[0]
                $passed = ($null -ne $content)
                $msg = "Response type: $($content.type)"
                if ($content.type -eq "image") {
                    $msg = "Received image visualization"
                }
                elseif ($content.type -eq "text") {
                    $msg = "Received text response ($($content.text.Length) chars)"
                }
                Write-TestResult -TestName "Data Viz - GeoJSON Points" -Passed $passed -Message $msg -DurationMs $result.DurationMs
            }
            else {
                Write-TestResult -TestName "Data Viz - GeoJSON Points" -Passed $false -Message $result.Error -DurationMs $result.DurationMs
            }
        }
    },
    @{
...
Write-Host "  TomTom MCP Server - Orbis Maps Test Suite" -ForegroundColor Yellow
Write-Host ("=" * 62) -ForegroundColor Yellow
Write-Host "  Target:  $BaseUrl" -ForegroundColor Yellow
Write-Host "  Backend: tomtom-orbis-maps (16 tools)" -ForegroundColor Yellow
...
Write-Host "`n"
Write-Host ("=" * 62) -ForegroundColor Yellow
Write-Host "  ORBIS TEST SUMMARY" -ForegroundColor Yellow
Write-Host ("-" * 62) -ForegroundColor Yellow
...
if ($Script:FailCount -gt 0) {
    Write-Host "ORBIS TESTS FAILED" -ForegroundColor Red
    exit 1
}
else {
    Write-Host "ALL ORBIS TESTS PASSED" -ForegroundColor Green
    exit 0
}
